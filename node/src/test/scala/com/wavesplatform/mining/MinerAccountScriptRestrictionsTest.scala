...
class MinerAccountScriptRestrictionsTest extends PropSpec with WithDomain {
  ...
  property("miner account can't have account script or dApp with verifier after RideV6 feature activation") {
    withDomain(DomainPresets.RideV6) { d =>
      ...
      d.appendBlock(setScript(accountScript))
      miner.getNextBlockGenerationOffset(minerAcc) should produce(errMsgAfterRideV6)
  ...
  private def errMsgAfterRideV6 =
    s"Account(${minerAcc.toAddress}) has account script or dApp with verifier function and therefore not allowed to forge blocks"
