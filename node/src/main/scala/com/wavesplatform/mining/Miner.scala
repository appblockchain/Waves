...
class MinerImpl(
  ...
  private def hasAllowedForMiningScript(account: KeyPair, blockchain: Blockchain): Boolean =
    if (blockchain.isFeatureActivated(BlockchainFeatures.RideV6)) {
      blockchain.accountScript(account.toAddress).forall { scriptInfo =>
        scriptInfo.script match {
          case ContractScriptImpl(_, dApp) if dApp.verifierFuncOpt.isEmpty => true
          case _ => false
        }
      }
    } else {
  ...
  private def checkScript(blockchain: Blockchain, account: KeyPair): Either[String, Unit] = {
    Either.cond(
      ...
      (),
      if (blockchain.isFeatureActivated(BlockchainFeatures.RideV6)) {
        s"Account(${account.toAddress}) has account script or dApp with verifier function and therefore not allowed to forge blocks"
      } else {
